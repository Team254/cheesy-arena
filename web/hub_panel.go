...
func (web *Web) hubPanelHandler(w http.ResponseWriter, r *http.Request) {
	if !web.userIsAdmin(w, r) {
		return
	}

	template, err := web.parseFiles("templates/hub_panel.html", "templates/base.html")
	if err != nil {
		handleWebErr(w, err)
		return
	}
...
func (web *Web) hubPanelFoulListHandler(w http.ResponseWriter, r *http.Request) {
	template, err := web.parseFiles("templates/hub_panel_foul_list.html")
	if err != nil {
		handleWebErr(w, err)
		return
	}
	...
	err = template.ExecuteTemplate(w, "hub_panel_foul_list", data)
	if err != nil {
		handleWebErr(w, err)
		return
	}
...
func (web *Web) hubPanelWebsocketHandler(w http.ResponseWriter, r *http.Request) {
	if !web.userIsAdmin(w, r) {
		return
	}
