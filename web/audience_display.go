...
// Renders the audience display to be chroma keyed over the video feed.
func (web *Web) fieldAudienceDisplayHandler(w http.ResponseWriter, r *http.Request) {
	if !web.enforceDisplayConfiguration(w, r, map[string]string{"background": "#0ff", "reversed": "false",
		"overlayLocation": "bottom"}) {
		return
	}

	template, err := web.parseFiles("templates/field_audience_display.html")
	if err != nil {
		handleWebErr(w, err)
		return
	}
	...
	err = template.ExecuteTemplate(w, "field_audience_display.html", data)
	if err != nil {
		handleWebErr(w, err)
		return
	}
...
// The websocket endpoint for the audience display client to receive status updates.
func (web *Web) fieldAudienceDisplayWebsocketHandler(w http.ResponseWriter, r *http.Request) {
	display, err := web.registerDisplay(r)
	if err != nil {
		handleWebErr(w, err)
		return
