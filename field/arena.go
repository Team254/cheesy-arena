...
func (arena *Arena) SetAudienceDisplayMode(mode string) {
	if arena.AudienceDisplayMode != mode {
		...
		if mode == "score" {
			...
			go arena.CompanionClient.SendEvent(partner.EventPostResult)
			...
			go arena.CompanionClient.SendEvent(partner.EventSetAudience)
		}
	}
}
