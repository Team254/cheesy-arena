...
func (arena *Arena) generateMatchTimeMessage() any {
	...
	currentShift := 0 // Default 0 (Non-Teleop Phase)
	shiftTimeSec := 0

	// Shift is calculated only when MatchState is Teleop (usually 5).
	if arena.MatchState == 5 {
		// Logical judgment based on the timetable you provided (total countdown seconds: 162 seconds).
		if time > 0 && time < 33 {
			currentShift = 1 // TRANSITION is tentatively set to 0, or can be set to 1 as needed.
	...
	return MatchTimeMessage{
		...
		arena.RedRealtimeScore.CurrentScore.HubActive,  // Fetch red alliance real-time Hub status
		arena.BlueRealtimeScore.CurrentScore.HubActive, // Fetch blue alliance real-time Hub status
		currentShift, // <--- Send the calculated Shift value
		shiftTimeSec,
	}
}
...
func (arena *Arena) GenerateScorePostedMessage() any {
	...
	// Define the RP variables generated by wins (RedWinRP / BlueWinRP).
	var redWinRP, blueWinRP int

	switch arena.SavedMatch.Status {
	case game.RedWonMatch:
		redRankingPoints += 3
		redWinRP = 3 // Red alliance wins 3 RP
	case game.BlueWonMatch:
		blueRankingPoints += 3
		blueWinRP = 3 // Blue alliance wins 3 RP
	case game.TieMatch:
		redRankingPoints++
		blueRankingPoints++
		redWinRP = 1 // Tie, each alliance gets 1 RP
		blueWinRP = 1
	}
