...
func TestPlayoffTournamentCreateMatchesAndBreaks(t *testing.T) {
	...
	if assert.Equal(t, 19, len(matches)) {
		...
		assertMatch(t, matches[4], 5, 7880, "Match 5", "M5", "Round 2 Lower", "M5", 0, 0, true, "sf", 5, 1)
		assertMatch(t, matches[5], 6, 8480, "Match 6", "M6", "Round 2 Lower", "M6", 0, 0, true, "sf", 6, 1)
		assertMatch(t, matches[6], 7, 9080, "Match 7", "M7", "Round 2 Upper", "M7", 0, 0, true, "sf", 7, 1)
		assertMatch(t, matches[7], 8, 9680, "Match 8", "M8", "Round 2 Upper", "M8", 0, 0, true, "sf", 8, 1)
		assertMatch(t, matches[8], 9, 10760, "Match 9", "M9", "Round 3 Lower", "M9", 0, 0, true, "sf", 9, 1)
		assertMatch(t, matches[9], 10, 11360, "Match 10", "M10", "Round 3 Lower", "M10", 0, 0, true, "sf", 10, 1)
		assertMatch(t, matches[10], 11, 12440, "Match 11", "M11", "Round 4 Upper", "M11", 0, 0, true, "sf", 11, 1)
		assertMatch(t, matches[11], 12, 13040, "Match 12", "M12", "Round 4 Lower", "M12", 0, 0, true, "sf", 12, 1)
		assertMatch(t, matches[12], 13, 14540, "Match 13", "M13", "Round 5 Lower", "M13", 0, 0, true, "sf", 13, 1)
		assertMatch(t, matches[13], 14, 16040, "Final 1", "F1", "", "F", 0, 0, false, "f", 1, 1)
		assertMatch(t, matches[14], 15, 17540, "Final 2", "F2", "", "F", 0, 0, false, "f", 1, 2)
		assertMatch(t, matches[15], 16, 19040, "Final 3", "F3", "", "F", 0, 0, false, "f", 1, 3)
		assertMatch(t, matches[16], 17, 19640, "Overtime 1", "O1", "", "F", 0, 0, true, "f", 1, 4)
		assertMatch(t, matches[17], 18, 20240, "Overtime 2", "O2", "", "F", 0, 0, true, "f", 1, 5)
		assertMatch(t, matches[18], 19, 20840, "Overtime 3", "O3", "", "F", 0, 0, true, "f", 1, 6)
	}
	for i := 0; i < 16; i++ {
	...
	assert.Nil(t, err)
	if assert.Equal(t, 7, len(scheduledBreaks)) {
		assertBreak(t, scheduledBreaks[0], 5, 7400, 480, "Field Break")
		assertBreak(t, scheduledBreaks[1], 9, 10280, 480, "Field Break")
		assertBreak(t, scheduledBreaks[2], 11, 11960, 480, "Field Break")
		assertBreak(t, scheduledBreaks[3], 13, 13640, 900, "Awards Break")
		assertBreak(t, scheduledBreaks[4], 14, 15140, 900, "Awards Break")
		assertBreak(t, scheduledBreaks[5], 15, 16640, 900, "Awards Break")
		assertBreak(t, scheduledBreaks[6], 16, 18140, 900, "Awards Break")
	}
	...
	if assert.Equal(t, 9, len(matches)) {
		...
		assertMatch(t, matches[4], 44, 4360, "Final 2", "F2", "", "F", 1, 0, false, "f", 1, 2)
		assertMatch(t, matches[5], 45, 5440, "Final 3", "F3", "", "F", 1, 0, false, "f", 1, 3)
		assertMatch(t, matches[6], 46, 6040, "Overtime 1", "O1", "", "F", 1, 0, true, "f", 1, 4)
		assertMatch(t, matches[7], 47, 6640, "Overtime 2", "O2", "", "F", 1, 0, true, "f", 1, 5)
		assertMatch(t, matches[8], 48, 7240, "Overtime 3", "O3", "", "F", 1, 0, true, "f", 1, 6)
	}
	for i := 0; i < 6; i++ {
		assert.Equal(t, game.MatchScheduled, matches[i].Status)
	}
	...
	if assert.Equal(t, 3, len(scheduledBreaks)) {
		...
		assertBreak(t, scheduledBreaks[1], 44, 3880, 480, "Field Break")
		assertBreak(t, scheduledBreaks[2], 45, 4960, 480, "Field Break")
	}
}
