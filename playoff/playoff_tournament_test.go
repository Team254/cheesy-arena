...
func TestPlayoffTournamentCreateMatchesAndBreaks(t *testing.T) {
	...
	if assert.Equal(t, 19, len(matches)) {
		...
		assertMatch(t, matches[8], 9, 9380, "Match 9", "M9", "Round 3 Lower", "M9", 0, 0, true, "sf", 9, 1)
		assertMatch(t, matches[9], 10, 9920, "Match 10", "M10", "Round 3 Lower", "M10", 0, 0, true, "sf", 10, 1)
		assertMatch(t, matches[10], 11, 10820, "Match 11", "M11", "Round 4 Upper", "M11", 0, 0, true, "sf", 11, 1)
		assertMatch(t, matches[11], 12, 11360, "Match 12", "M12", "Round 4 Lower", "M12", 0, 0, true, "sf", 12, 1)
		assertMatch(t, matches[12], 13, 12560, "Match 13", "M13", "Round 5 Lower", "M13", 0, 0, true, "sf", 13, 1)
		assertMatch(t, matches[13], 14, 13760, "Final 1", "F1", "", "F", 0, 0, false, "f", 1, 1)
		assertMatch(t, matches[14], 15, 14960, "Final 2", "F2", "", "F", 0, 0, false, "f", 1, 2)
		assertMatch(t, matches[15], 16, 16160, "Final 3", "F3", "", "F", 0, 0, false, "f", 1, 3)
		assertMatch(t, matches[16], 17, 16460, "Overtime 1", "O1", "", "F", 0, 0, true, "f", 1, 4)
		assertMatch(t, matches[17], 18, 17060, "Overtime 2", "O2", "", "F", 0, 0, true, "f", 1, 5)
		assertMatch(t, matches[18], 19, 17660, "Overtime 3", "O3", "", "F", 0, 0, true, "f", 1, 6)
	}
	for i := 0; i < 16; i++ {
	...
	if assert.Equal(t, 6, len(scheduledBreaks)) {
		assertBreak(t, scheduledBreaks[0], 9, 9080, 300, "Field Break")
		assertBreak(t, scheduledBreaks[1], 11, 10220, 600, "Field Break")
		assertBreak(t, scheduledBreaks[2], 13, 11660, 900, "Awards Break")
		assertBreak(t, scheduledBreaks[3], 14, 12860, 900, "Awards Break")
		assertBreak(t, scheduledBreaks[4], 15, 14060, 900, "Awards Break")
		assertBreak(t, scheduledBreaks[5], 16, 15260, 900, "Awards Break *")
	}
