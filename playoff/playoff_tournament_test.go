...
func TestPlayoffTournamentCreateMatchesAndBreaks(t *testing.T) {
	...
	if assert.Equal(t, 19, len(matches)) {
		...
		assertMatch(t, matches[1], 2, 5480, "Match 2", "M2", "Round 1 Upper", "M2", 4, 5, true, "sf", 2, 1)
		assertMatch(t, matches[2], 3, 5960, "Match 3", "M3", "Round 1 Upper", "M3", 2, 7, true, "sf", 3, 1)
		assertMatch(t, matches[3], 4, 6440, "Match 4", "M4", "Round 1 Upper", "M4", 3, 6, true, "sf", 4, 1)
		assertMatch(t, matches[4], 5, 7220, "Match 5", "M5", "Round 2 Lower", "M5", 0, 0, true, "sf", 5, 1)
		assertMatch(t, matches[5], 6, 7700, "Match 6", "M6", "Round 2 Lower", "M6", 0, 0, true, "sf", 6, 1)
		assertMatch(t, matches[6], 7, 8180, "Match 7", "M7", "Round 2 Upper", "M7", 0, 0, true, "sf", 7, 1)
		assertMatch(t, matches[7], 8, 8660, "Match 8", "M8", "Round 2 Upper", "M8", 0, 0, true, "sf", 8, 1)
		...
		assertMatch(t, matches[10], 11, 10700, "Match 11", "M11", "Round 4 Upper", "M11", 0, 0, true, "sf", 11, 1)
	...
	if assert.Equal(t, 7, len(scheduledBreaks)) {
		assertBreak(t, scheduledBreaks[0], 5, 6740, 480, "Field Break")
		assertBreak(t, scheduledBreaks[1], 9, 8960, 480, "Field Break")
		assertBreak(t, scheduledBreaks[2], 11, 10220, 480, "Field Break")
		assertBreak(t, scheduledBreaks[3], 13, 11480, 900, "Awards Break")
		assertBreak(t, scheduledBreaks[4], 14, 12680, 900, "Awards Break")
		assertBreak(t, scheduledBreaks[5], 15, 13880, 900, "Awards Break")
		assertBreak(t, scheduledBreaks[6], 16, 15080, 900, "Awards Break")
	}
