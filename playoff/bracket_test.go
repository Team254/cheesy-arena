...
func TestNewBracketErrors(t *testing.T) {
	_, err := newBracket(nil, []matchupTemplate{}, newMatchupKey(33, 12), 8)
	...
	_, err = newBracket(nil, []matchupTemplate{matchTemplate}, newMatchupKey(1, 1), 8)
	if assert.NotNil(t, err) {
...
func TestNewBracketInverseSeeding(t *testing.T) {
	...
	bracket, err := newBracket(database, matchupTemplates, newMatchupKey(3, 1), 2)
	assert.Nil(t, err)
...
func TestBracketUpdateTiming(t *testing.T) {
	...
	assert.Nil(t, bracket.Update(&startTime))
	matches, err := database.GetMatchesByType(model.Playoff)
	assert.Nil(t, err)
