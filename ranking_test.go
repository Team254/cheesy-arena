...
func TestRankingCrud(t *testing.T) {
	...
	ranking := Ranking{254, 1, 20, 625, 90, 554, 10, 50, 0.254, 3, 2, 1, 0, 10}
...
func TestCalculateRankings(t *testing.T) {
	...
	if assert.Equal(t, 6, len(rankings)) {
		assert.Equal(t, Ranking{4, 1, 5, 678, 213, 300, 200, 58, 0.897169713149801, 1, 0, 1, 0, 2}, rankings[0])
		assert.Equal(t, Ranking{5, 2, 6, 678, 213, 300, 200, 58, 0.2885856518054551, 1, 0, 2, 0, 3}, rankings[1])
		assert.Equal(t, Ranking{6, 3, 4, 433, 133, 200, 150, 18, 0.16735444255905835, 1, 0, 1, 0, 2}, rankings[2])
		assert.Equal(t, Ranking{1, 4, 4, 435, 160, 200, 100, 80, 0.2730468047134829, 0, 1, 2, 0, 3}, rankings[3])
		assert.Equal(t, Ranking{3, 5, 2, 190, 80, 100, 50, 40, 0.9026048462705047, 0, 1, 1, 0, 2}, rankings[4])
		assert.Equal(t, Ranking{2, 6, 3, 245, 80, 100, 50, 40, 0.8497802817628735, 0, 0, 2, 1, 3}, rankings[5])
	}
	...
	assert.Nil(t, err)
	if assert.Equal(t, 6, len(rankings)) {
		assert.Equal(t, Ranking{6, 1, 6, 866, 266, 400, 300, 36, 0.24043190328608438, 2, 0, 0, 0, 2}, rankings[0])
		assert.Equal(t, Ranking{5, 2, 8, 1111, 346, 500, 350, 76, 0.2092018731282357, 2, 0, 1, 0, 3}, rankings[1])
		assert.Equal(t, Ranking{4, 3, 5, 678, 213, 300, 200, 58, 0.5102423328818813, 1, 0, 1, 0, 2}, rankings[2])
		assert.Equal(t, Ranking{1, 4, 4, 625, 240, 300, 150, 120, 0.284824110942037, 0, 2, 1, 0, 3}, rankings[3])
		assert.Equal(t, Ranking{3, 5, 2, 380, 160, 200, 100, 80, 0.6930700440076261, 0, 2, 0, 0, 2}, rankings[4])
		assert.Equal(t, Ranking{2, 6, 3, 435, 160, 200, 100, 80, 0.4018978925803393, 0, 1, 1, 1, 3}, rankings[5])
	}
}
...
func TestSortRankings(t *testing.T) {
	...
	rankings[1] = &Ranking{1, 0, 50, 50, 50, 50, 50, 50, 0.49, 3, 2, 1, 0, 10}
	rankings[2] = &Ranking{2, 0, 50, 50, 50, 50, 50, 50, 0.51, 3, 2, 1, 0, 10}
	rankings[3] = &Ranking{3, 0, 50, 50, 50, 50, 50, 49, 0.50, 3, 2, 1, 0, 10}
	rankings[4] = &Ranking{4, 0, 50, 50, 50, 50, 50, 51, 0.50, 3, 2, 1, 0, 10}
	rankings[5] = &Ranking{5, 0, 50, 50, 50, 50, 49, 50, 0.50, 3, 2, 1, 0, 10}
	rankings[6] = &Ranking{6, 0, 50, 50, 50, 50, 51, 50, 0.50, 3, 2, 1, 0, 10}
	rankings[7] = &Ranking{7, 0, 50, 50, 50, 49, 50, 50, 0.50, 3, 2, 1, 0, 10}
	rankings[8] = &Ranking{8, 0, 50, 50, 50, 51, 50, 50, 0.50, 3, 2, 1, 0, 10}
	rankings[9] = &Ranking{9, 0, 50, 50, 49, 50, 50, 50, 0.50, 3, 2, 1, 0, 10}
	rankings[10] = &Ranking{10, 0, 50, 50, 51, 50, 50, 50, 0.50, 3, 2, 1, 0, 10}
	rankings[11] = &Ranking{11, 0, 50, 49, 50, 50, 50, 50, 0.50, 3, 2, 1, 0, 10}
	rankings[12] = &Ranking{12, 0, 50, 51, 50, 50, 50, 50, 0.50, 3, 2, 1, 0, 10}
	rankings[13] = &Ranking{13, 0, 49, 50, 50, 50, 50, 50, 0.50, 3, 2, 1, 0, 10}
	rankings[14] = &Ranking{14, 0, 51, 50, 50, 50, 50, 50, 0.50, 3, 2, 1, 0, 10}
	sortedRankings := sortRankings(rankings)
	assert.Equal(t, 14, sortedRankings[0].TeamId)
	assert.Equal(t, 12, sortedRankings[1].TeamId)
	assert.Equal(t, 10, sortedRankings[2].TeamId)
	assert.Equal(t, 8, sortedRankings[3].TeamId)
	assert.Equal(t, 6, sortedRankings[4].TeamId)
	assert.Equal(t, 4, sortedRankings[5].TeamId)
	assert.Equal(t, 2, sortedRankings[6].TeamId)
	assert.Equal(t, 1, sortedRankings[7].TeamId)
	assert.Equal(t, 3, sortedRankings[8].TeamId)
	assert.Equal(t, 5, sortedRankings[9].TeamId)
	assert.Equal(t, 7, sortedRankings[10].TeamId)
	assert.Equal(t, 9, sortedRankings[11].TeamId)
	assert.Equal(t, 11, sortedRankings[12].TeamId)
	assert.Equal(t, 13, sortedRankings[13].TeamId)

	// Check with unequal number of matches played.
	rankings = make(map[int]*Ranking)
	rankings[1] = &Ranking{1, 0, 10, 25, 25, 25, 25, 25, 0.49, 3, 2, 1, 0, 5}
	rankings[2] = &Ranking{2, 0, 19, 50, 50, 50, 50, 50, 0.51, 3, 2, 1, 0, 9}
	rankings[3] = &Ranking{3, 0, 20, 50, 50, 50, 50, 50, 0.51, 3, 2, 1, 0, 10}
	sortedRankings = sortRankings(rankings)
